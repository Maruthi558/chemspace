...
export default function CopilotWindow({ onClose }) {
  ...
  const handleSend = async (textOverride = null, isRegenerate = false) => {
    ...
    try {
      const result = await aiCopilot.sendMessage(fullPrompt, { currentPath: location.pathname }, controller.signal);
      ...
      // If TTS enabled or voice was used, speak the response
      if (ttsEnabled || micState === 'processing') {
